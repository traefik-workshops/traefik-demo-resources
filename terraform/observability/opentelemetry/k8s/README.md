...
| <a name="input_langsmith_host_filter"></a> [langsmith\_host\_filter](#input\_langsmith\_host\_filter) | Allow-list of host patterns (RE2 regex, matched against span server.address). When set, LangSmith gets a dedicated tail-sampled traces pipeline that exports a trace only if one of its spans matches; other backends still get every trace. Empty = export all. Example: `["ai.localhost"]`. | `list(string)` | `[]` | no |
