...
%{ if enable_preview_mode ~}
  - |
    ...
    # Prefer the distro's docker.io: the download.docker.com apt repo is often slow/unreachable
    # behind a just-booted NAT (and the AMI may already ship docker).
    apt-get update || true
    apt-get install -y docker.io || apt-get install -y docker-ce docker-ce-cli containerd.io || true
