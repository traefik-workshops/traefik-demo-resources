...
```hcl
module "traefik" {
  source = "git::https://github.com/traefik-workshops/traefik-demo-resources.git//terraform/traefik/k8s?ref=v8.0.0"

  name              = "traefik"
  namespace         = "traefik"
  traefik_hub_token = var.traefik_hub_token

  # Default TLSStore with an ACME-generated wildcard cert — rendered as chart
  # values tlsStore.default.defaultGeneratedCert.{resolver,domain.main}, so no
  # hand-rolled TLSStore manifest is needed. `resolver` must name a configured
  # certificates resolver (the module's Cloudflare paths create "cf").
...
| <a name="input_acme_disable_ans_checks"></a> [acme\_disable\_ans\_checks](#input\_acme\_disable\_ans\_checks) | Skip the DNS-01 pre-check's direct queries to the zone's authoritative nameservers; rely on acme\_dns\_resolvers only (see traefik/shared). | `bool` | `false` | no |
| <a name="input_acme_disable_propagation_checks"></a> [acme\_disable\_propagation\_checks](#input\_acme\_disable\_propagation\_checks) | Skip lego's own DNS-01 propagation checks; rely on the delay + the CA (see traefik/shared). | `bool` | `false` | no |
| <a name="input_acme_dns_resolvers"></a> [acme\_dns\_resolvers](#input\_acme\_dns\_resolvers) | Recursive resolvers for the DNS-01 propagation pre-check (host:port). Default: Cloudflare's public resolvers; pass reachable ones on networks that block them (see traefik/shared). | `list(string)` | <pre>[<br/>  "1.1.1.1:53",<br/>  "1.0.0.1:53"<br/>]</pre> | no |
| <a name="input_acme_propagation_delay_seconds"></a> [acme\_propagation\_delay\_seconds](#input\_acme\_propagation\_delay\_seconds) | Seconds lego waits after placing the DNS-01 TXT record before checks/validation (propagation.delayBeforeChecks; see traefik/shared). | `number` | `20` | no |
...
| <a name="input_redis_persistence"></a> [redis\_persistence](#input\_redis\_persistence) | Back the API Management Redis with a PersistentVolumeClaim (8Gi RWO) instead of emptyDir. Default FALSE — the claim is a real cloud disk that outlives `terraform destroy`, and this Redis holds only plan rate-limit/quota counters, never ACME certificates. See the note in ../../tools/redis/k8s/main.tf. | `bool` | `false` | no |
