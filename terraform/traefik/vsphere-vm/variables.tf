...
variable "extra_labels" {
  description = "Extra Traefik labels merged into the VM's own Notes (on top of the dashboard self-registration labels, when enabled), rendered as LINE-format `traefik.key=value` labels for the gateway's own vsphere provider."
  type        = map(string)
  default     = {}
}
...
variable "enable_dashboard_discovery" {
  description = "Self-register the Traefik VM through its own Notes (a traefik.enable + dashboard router/service label block) so its OWN vsphere provider discovers the dashboard as dashboard@vsphere. Disable when the dashboard is advertised another way (e.g. a file-rule uplink) so the VM isn't self-discovered at all."
  type        = bool
  default     = true
}
