...
module "traefik_vsphere" {
  source = "git::https://github.com/traefik-workshops/traefik-demo-resources.git//terraform/traefik/vsphere-vm?ref=v8.0.0"
...
| <a name="input_network"></a> [network](#input\_network) | Name of the port group / network the VM's NIC joins. DHCP is assumed unless network\_config gives the VM a static address; either way the parent dials the VM's guest IP :9443 in-network. | `string` | n/a | yes |
...
| <a name="input_network_config"></a> [network\_config](#input\_network\_config) | cloud-init network-config v2 document for the gateway VM (ONE document, not keyed — this module clones exactly one VM), passed through to compute/vsphere/vm and applied at BOOT. Set it when the network has no DHCP, or when the parent must know the child's uplink address at PLAN time (a hub that references this module's private\_ips cannot be ordered behind a DNS gate the child itself depends on). Empty = DHCP. | `any` | `{}` | no |
