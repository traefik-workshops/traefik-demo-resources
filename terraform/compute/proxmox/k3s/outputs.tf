...
output "node_ip" {
  description = "The VM's guest IP (QEMU-agent-reported) — also where klipper (k3s servicelb) publishes LoadBalancer Services, so point demo DNS / /etc/hosts entries here"
  value       = local.node_ip
}
...
output "vm_id" {
  description = "Proxmox VMID of the k3s VM"
