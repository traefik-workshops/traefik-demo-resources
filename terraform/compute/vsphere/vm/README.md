...
Role-agnostic by design: cloud-init is rendered by the caller and passed in as opaque `user_data`; the Hub vsphere provider's label block (line-format `traefik.<key>=<value>` in the VM **Notes**) is rendered by the caller and passed in as opaque `annotation` text, one per instance key. Outputs mirror the other `compute/*` modules (`instances` / `private_ips` / `public_ips`) — on vSphere both IP maps carry the same primary guest address.

## Example usage

```hcl
module "vm" {
  source = "git::https://github.com/traefik-workshops/traefik-demo-resources.git//terraform/compute/vsphere/vm?ref=v8.0.0"
  ...
  # The VM Notes: the Hub vsphere provider's label block, one traefik.k=v per line.
  annotation = {
    "whoami-1" = local.traefik_label_block
    "whoami-2" = local.traefik_label_block
  }
}
```
...
| <a name="input_annotation"></a> [annotation](#input\_annotation) | VM Notes (config.annotation) per instance key. The Hub vsphere provider reads a LINE-FORMAT label block from it — one `traefik.<key>=<value>` per line — so the caller renders the block here. An absent key leaves the Notes untouched. Updatable in place: a label change is a reconfigure, never a replacement. | `map(string)` | `{}` | no |
...
| <a name="input_extra_config"></a> [extra\_config](#input\_extra\_config) | Extra guestinfo extraConfig entries merged onto each VM, keyed by instance key (e.g. { "<app>-1" = { "guestinfo.role" = "worker" } }). Empty per-key map adds nothing. | `map(map(string))` | `{}` | no |
...
| <a name="input_tags"></a> [tags](#input\_tags) | vCenter tag IDs to attach, keyed by instance key. The vsphere provider has no standalone attach resource — tags ride the VM resource itself, so they are set at create. Organisational only: the Hub vsphere provider does not read tags (see var.annotation). | `map(list(string))` | `{}` | no |
