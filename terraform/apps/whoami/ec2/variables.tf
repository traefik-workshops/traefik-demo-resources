...
variable "whoami_version" {
  description = "The Whoami version to install. Must be a real traefik/whoami image tag — they carry a `v` prefix (e.g. v1.10.1); a bare `1.10.1` is `manifest unknown` and the binary extraction silently fails."
  type        = string
  default     = "v1.10.1"
}
