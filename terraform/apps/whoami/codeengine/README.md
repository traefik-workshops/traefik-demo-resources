...
## There is NO traefik config surface — by design

Code Engine apps have no user-settable labels or annotations, so the Traefik Hub `ibmCodeEngine` provider is **config-less**: every ready app becomes a service targeting its HTTPS endpoint URL (URL-mode — no IP/port discovery), routed by the provider's `defaultRule`. The only knobs live on the provider side (`constraints` matched against the synthesized pseudo-labels `name` and `visibility`, plus `exposedByDefault`). This module therefore has no `tags`/`annotations` inputs at all.
...
module "whoami_codeengine" {
  ...
  apps = {
    whoami = {
      port = 80
      name = "whoami-codeengine" # body shows `Name: whoami-codeengine`
    }
  }
}
