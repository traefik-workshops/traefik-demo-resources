output "rendered" {
  ...
  value = templatefile("${path.module}/cloud-init.tpl", {
    ...
    collector_gate = local.otlp_address != "" ? templatefile("${path.module}/../../../cloud-init-snippets/otlp-collector-gate.sh.tpl", { otlp_address = local.otlp_address, rounds = 180, verify_tls = false }) : ""
  })
}
